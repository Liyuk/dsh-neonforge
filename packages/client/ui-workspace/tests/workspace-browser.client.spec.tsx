...
describe('WorkspaceBrowser', () => {
  ...
  it('renders the grouped tree by default and switches to the flat list via Group by', () => {
    ...
    expect(screen.getByText('alpha').closest('[role="treeitem"]')?.hasAttribute('data-neonforge-workspace-folder')).toBe(true)
  ...
  it('marks the current session row as the Neonforge navigation ticket', () => {
    mount({
      useSessions: hook(sessionState([summary('alpha-s', 1)], { current: sid('alpha-s') })),
      useWorkspaces: hook(workspaceState([workspace('alpha', ['alpha-s'])])),
    })

    const row = screen.getByText('alpha-s').closest('[role="treeitem"]')
    expect(row?.hasAttribute('data-neonforge-session-ticket')).toBe(true)
    expect(row?.hasAttribute('data-neonforge-selected')).toBe(true)
    expect(screen.getByText('alpha-s').hasAttribute('data-neonforge-ticket-title')).toBe(true)
  })
