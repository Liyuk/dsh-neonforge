...
/** Compact workspace-card date matching the sidebar's fixed YYYY-MM-DD stamp. */
function workspaceDateLabel(createdAt: number): string {
  const d = new Date(createdAt)
  const pad2 = (v: number): string => String(v).padStart(2, '0')
...
export function ProjectRowItem({ group, onToggle, onCreate, actions, drag, home, t }: {
  ...
  const ownRow = (
    <div
      ...
      role="treeitem"
      data-neonforge-workspace-folder
      data-neonforge-workspace-selected={group.containsCurrent || undefined}
      ...
      onDragEnd={drag?.end}
    >
      <span className={clsx(css.slot, css.folder, active && css.folderActive)} data-neonforge-folder-icon>
        {row.expanded ? <IconFolderOpen16 /> : <IconFolderClose16 />}
      </span>
      <span className={clsx(css.slot, css.chevron)}>
        <IconTriangleRightFill14 className={clsx(css.arrow, row.expanded && css.arrowOpen)} />
      </span>
      <span className={css.projectText} data-neonforge-project-text>
        <span className={css.title}>{label}</span>
        {row.createdAt !== undefined && (
          <span className={css.projectMeta} data-neonforge-project-meta>
            <span>{row.sessionCount} 个会话</span>
            <span>{workspaceDateLabel(row.createdAt)}</span>
          </span>
        )}
      </span>
...
export function SessionNodeItem({ node, currentId, now, onOpen, onRename, onFork, onArchive, drag, flat = false, t }: {
  ...
  const ownRow = (
    <div
      ...
      role="treeitem"
      aria-selected={selected}
      data-neonforge-session-ticket
      data-neonforge-selected={selected || undefined}
    ...
    >
      ...
      <span className={css.title} data-neonforge-ticket-title>{title}</span>
