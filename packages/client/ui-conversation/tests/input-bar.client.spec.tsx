...
describe('running and lock semantics', () => {
  ...
  it('exposes Neonforge hooks for the readable composer controls', () => {
    const { view, button, textarea } = bench({ draft: 'go' })
    expect(view.container.querySelector('[data-neonforge-composer]')).not.toBeNull()
    expect(textarea.hasAttribute('data-neonforge-composer-input')).toBe(true)
    expect(button.hasAttribute('data-neonforge-send-button')).toBe(true)
  })
