...
window.__ModuleLoader__.load({
  ...
  factory: function () {
    ...
    let autoDarkApplied = false
    let skinSelected = false
    const enabled = () => window.localStorage.getItem(toggleKey) !== '0'
    const scheme = () => theme?.getTheme().active.colorScheme === 'light' ? 'light' : 'dark'
    ...
    const setEnabled = value => {
      ...
      if (value) {
        ...
        if (theme && autoDarkApplied) theme.setTheme(previousPreference)
        autoDarkApplied = false
      }
      updateSkinControls()
    }
    const mountSetting = () => {
    ...
    function apply(ctx) {
      ...
      if (enabled() && previousPreference === 'system') {
        autoDarkApplied = true
        theme.setTheme('dark')
      }
      if (enabled()) {
        document.body.setAttribute('data-dsh-neonforge', '')
      ...
      ctx.on('theme/change', snapshot => {
        if (!enabled()) return
        if (snapshot.preference !== 'dark') autoDarkApplied = false
        const next = snapshot.active.colorScheme === 'light' ? 'light' : 'dark'
        document.body.setAttribute('data-neonforge-scheme', next)
        updateSkinControls()
      })
      ...
      ctx.effect(() => () => {
        ...
        document.body.removeAttribute('data-neonforge-scheme')
        if (autoDarkApplied) theme?.setTheme(previousPreference)
        autoDarkApplied = false
